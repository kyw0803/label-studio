...
def project_list(request):
    ...
    logger.error(f"DEBUG: [project_list] View reached!")
    ...
    logger.error(f"DEBUG: [project_list] request.session._session_key: {getattr(request.session, '_session_key', 'NO ATTR')}")
    logger.error(f"DEBUG: [project_list] request.session.modified: {request.session.modified}")
    logger.error(f"DEBUG: [project_list] request.session._session_cache: {getattr(request.session, '_session_cache', 'NO ATTR')}")
    
    # _cached_user가 있으면 강제로 request.user에 설정
    if hasattr(request, '_cached_user') and request._cached_user:
        logger.error(f"DEBUG: [project_list] Setting request.user from _cached_user")
        request.user = request._cached_user
    
    logger.error(f"DEBUG: [project_list] After fix - request.user: {request.user}, authenticated: {request.user.is_authenticated}")
